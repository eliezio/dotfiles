...
**Daily apply**:
After bootstrap, the user runs `chezmoi apply` directly. No wrapper. Templates are pure (no secret-decryption at template-resolution time); the only sops invocation is in `run_onchange_after_gh_setup.sh.tmpl` at script-execution time.
...
> **A:** And secrets?
>
> **B:** `secrets.yaml` is encrypted with sops to a single age recipient. The private key (`~/.config/sops/age/keys.txt`) is restored manually from a password manager — never in the repo. `chezmoi apply` resolves templates without touching sops; the only sops call is in `run_onchange_after_gh_setup.sh.tmpl`, which decrypts at execution time and pipes the token into `gh auth login`. It's `onchange`, not unconditional: the rendered body embeds the ciphertext's sha256, so it re-runs only when `secrets.yaml` is re-encrypted (which is when tokens actually rotate). `chezmoi diff` / `status` / `cat` never call sops.
